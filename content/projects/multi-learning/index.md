...
### 最终效果：
{{< figure src="judgement.png" alt="三种策略准确率对比" caption="三种策略准确率对比：超分后再分类显著恢复了低分辨率图像的分类效果。" align="center" width="720" >}}
...
{{< collapse summary="展开 task2 初试的 CNN baseline 代码" >}}
```python
#定义模型
class Net(nn.Module):
    def __init__(self):
...
{{< collapse summary="展开第一版 SRCNN 代码" >}}
```python
class Net(nn.Module):
...
具体表现在Dataset之中：
{{< collapse summary="展开 DIV2K Dataset 切片代码" >}}
```python
...
{{< collapse summary="展开 CIFAR10 分类模型代码" >}}
```python
#encoder
...
{{< figure src="Residual_block.png" alt="Residual block 结构示意图" caption="Residual block 通过跳跃连接保留原始特征，缓解深层网络训练中的梯度问题。" align="center" width="360" >}}

以下为修改后的模型代码：
{{< collapse summary="展开 SRNet 残差块模型代码" >}}
```python
#Encoder
class Encoder(nn.Module):
    def __init__(self):
...
{{< collapse summary="展开 MultiTaskNet 串联结构代码" >}}
```python
