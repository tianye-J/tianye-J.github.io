...
require 'home-section-header h2[[:space:]]*\{[^}]*font-size:[[:space:]]*clamp\(28px,[[:space:]]*8vw,[[:space:]]*32px\)' "$css" 'mobile homepage section titles must use the compact editorial scale'
require 'home-section-entry h3[[:space:]]*\{[^}]*font-size:[[:space:]]*clamp\(18px,[[:space:]]*5\.4vw,[[:space:]]*21px\)' "$css" 'mobile homepage article titles must stay below section-title scale'
require 'home-section\.home-archive-band::before[[:space:]]*\{[^}]*font-size:[[:space:]]*clamp\(56px,[[:space:]]*15vw,[[:space:]]*60px\)' "$css" 'mobile archive numbering must be visually quieter'
require 'home-section-entry a[[:space:]]*\{[^}]*padding:[[:space:]]*13px[[:space:]]*44px[[:space:]]*14px[[:space:]]*0' "$css" 'mobile homepage entries need right-side safe space for the floating top link'
require 'top-link[[:space:]]*\{[^}]*width:[[:space:]]*40px[^}]*height:[[:space:]]*40px[^}]*opacity:[[:space:]]*0\.86' "$css" 'mobile top-link must be smaller and visually lighter'
