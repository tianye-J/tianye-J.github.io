...
+++
> 面向零基础读者。读完后，你会知道仿真服务器到底负责什么，也能开始顺着 RCSSServerMJ 的源码往下看。
>
...
先想象一个虚拟世界：里面有地面、一个足球、一个机器人。你把球推一下，它会滚动、减速、停下；机器人抬脚，重心会偏移，也可能摔倒。
...
为什么要分开？因为 RoboCup 比赛里，每支队伍只提交自己的**客户端**程序，所有客户端都连到同一个官方服务器。服务器提供同一套“裁判和物理世界”，客户端才是各队自己的算法。

### 数据流

{{< figure src="/images/robocup/rcssservermj-client-server-architecture.png" alt="RCSSServerMJ Client-Server 架构图" caption="RCSSServerMJ 统一运行物理仿真和裁判规则，每个 Client 接收感知并返回动作。" align="center" width="900" >}}
...
类比一下：TCP 像打电话，先拨号建立连接，然后双方持续对话，挂断才断开。UDP 更像发短信：直接发，不保证对方收到。
...
{{< figure src="/images/robocup/rcssservermj-length-prefixed-message.png" alt="RCSSServerMJ 长度前缀消息格式图" caption="长度前缀协议：先读取 4 字节消息长度，再按长度读取完整的 S-expression 消息体。" align="center" width="900" >}}
...
实时仿真程序通常都绕着一个 **无限循环** 跑，每轮做一组固定操作：
...
打开 `server/server.py`，找到 `run()` 方法，简化后大概是这样：
...
机器人的关节由电机驱动。不能直接把关节瞬间“设置”到某个角度（就像你不能瞬间把胳膊移到任意位置），而是要通过电机施加**力矩（torque）**来驱动关节旋转。
...
```text
类比：用橡皮筋（kp）把关节往目标位置拉，再加一个阻尼器（kd）防止它来回弹跳。
```
...
现在把前面的概念串起来，看一个球员从启动到踢球会经历什么：

**阶段 1：启动与连接**

1. Janus 客户端从 `run_player.py` 启动。
2. 客户端创建 TCP Socket，连接到 Server 的 `127.0.0.1:60000`。
3. 客户端发送初始化消息，例如 `(init T1 MujocoCodebase 1)`，告诉 Server 机器人型号、队名和球员号。
4. Server 收到 `init` 后，在物理世界中生成一个 T1 机器人，并让 Agent 进入可参与比赛的状态。

**阶段 2：主循环（每 20ms 一轮）**

{{< figure src="/images/robocup/rcssservermj-server-client-loop.png" alt="RCSSServerMJ 与 Janus 的每帧通信循环图" caption="Server 与 Client 每 20ms 交换一次感知和动作：Server 推进物理并发送感知，Janus 更新状态并返回动作。" align="center" width="900" >}}
