...
require '\.post-content[[:space:]]*\{[^}]*font-size:[[:space:]]*16px[^}]*line-height:[[:space:]]*1\.72[^}]*letter-spacing:[[:space:]]*0\.008em' "$css" 'desktop article typography must use the compact reading scale'
require '\.post-content p[[:space:]]*\{[^}]*margin-block:[[:space:]]*0\.85em' "$css" 'article paragraphs must use the compact reading rhythm'
require '\.post-content h2[[:space:]]*\{[^}]*margin:[[:space:]]*2em 0 0\.7em[^}]*font-size:[[:space:]]*26px' "$css" 'article h2 headings must use the compact editorial scale'
require '\.post-content h3[[:space:]]*\{[^}]*margin:[[:space:]]*1\.75em 0 0\.55em[^}]*font-size:[[:space:]]*21px' "$css" 'article h3 headings must use the compact editorial scale'
require '\.post-content li[[:space:]]*\{[^}]*margin-bottom:[[:space:]]*0\.22em' "$css" 'article list items must use compact vertical spacing'
require '\.post-content p:has\(> img:only-child\),[[:space:]]*\n\.post-content figure[[:space:]]*\{[^}]*margin:[[:space:]]*1\.4em 0' "$css" 'article image wrappers must share compact desktop spacing'
require '\.post-content p > img:only-child,[[:space:]]*\n\.post-content figure img[[:space:]]*\{[^}]*max-width:[[:space:]]*min\(100%,[[:space:]]*680px\)[^}]*max-height:[[:space:]]*64vh[^}]*object-fit:[[:space:]]*contain' "$css" 'article images must be bounded by width and viewport height'
reject '\.post-content img[[:space:]]*\{[^}]*margin:[[:space:]]*2rem 0' "$css" 'legacy article image spacing must not override the compact rhythm'
require '\.post-content[[:space:]]*\{[^}]*font-size:[[:space:]]*16px[^}]*line-height:[[:space:]]*1\.7[^}]*letter-spacing:[[:space:]]*0\.008em' "$css" 'mobile article typography must preserve a readable compact scale'
require '\.post-content h2[[:space:]]*\{[^}]*font-size:[[:space:]]*23px' "$css" 'mobile article h2 headings must use the compact scale'
require '\.post-content h3[[:space:]]*\{[^}]*font-size:[[:space:]]*19px' "$css" 'mobile article h3 headings must use the compact scale'
require '\.post-content p:has\(> img:only-child\),[[:space:]]*\n[[:space:]]*\.post-content figure[[:space:]]*\{[^}]*margin:[[:space:]]*1\.1em 0' "$css" 'mobile article images must use compact vertical spacing'
